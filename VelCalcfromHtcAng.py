"""This code calculates velocity[m/s] from angle[deg] and heat transfer coefficient[W/m^2-K] from data using a FluxTeq PHFS-01 heat flux sensor. 
Populated with data from AngleHtcVel.csv
...
df = pd.read_csv('AngleHtcVel.csv', parse_dates=True)

# define the number of x and y bins
x_bin_N = 50
y_bin_N = 50

# Define the points where you want to interpolate to find a velocity [m/s]
x_interpolate = 0 # Replace with your angle
y_interpolate = 40 # Replace with your heat transfer coefficient [W/m^2-K]
